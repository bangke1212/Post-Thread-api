...
export function updateTokenUserId(userId: string): void {
  const s = loadState();
...
export function loadToken(secret: string): Token | undefined {
  const s = loadState();
  const token = s.tokens.find(function(t) { return t.id === 1; });
...
export function savePostRecord(post: Omit<Post, 'id'>): number {
  const s = loadState();
...
export function startRunRecord(): number {
  const s = loadState();
...
export function countRecentFailures(last: number = 3): number {
  const s = loadState();
...
export function getUsedImageIds(): Set<string> {
  const s = loadState();
  return new Set(s.used_images.map(function(r) { return r.image_id; }));
}

export function markImageUsed(imageId: string, imageUrl: string): void {
  const s = loadState();
  s.used_images.push({
    image_id: imageId,
    image_url: imageUrl,
