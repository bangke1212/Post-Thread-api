...
async function fetchTweetByUrl(tweetUrl) {
  const tweetId = tweetUrl.split('/status/')[1] ? tweetUrl.split('/status/')[1].replace(/[^0-9]/g,'') : '';
...
async function fetchLatestTweet(username) {
  ...
  for (const base of nitterInstances) {
    try {
      const r = await fetch(base + '/' + username + '/rss', { headers: { 'User-Agent': 'Mozilla/5.0' } });
      if (r.ok) {
        const x = await r.text();
        const im = x.match(/<item>([\s\S]*?)<\/item>/);
        if (im) {
          const i = im[1];
          const lk = (i.match(/<link>(.*?)<\/link>/) || [])[1] || '';
          const id = lk.split('/status/')[1] ? lk.split('/status/')[1].replace(/[^0-9]/g,'') : '0';
          const tx = ((i.match(/<title>(.*?)<\/title>/) || [])[1] || '').replace(/<!\[CDATA\[|\]\]>/g,'').trim();
          const dt = (i.match(/<pubDate>(.*?)<\/pubDate>/) || [])[1] || new Date().toISOString();
          if (tx) return { id, text: tx.slice(0,500), created_at: new Date(dt).toISOString(), url: 'https://x.com/' + username + '/status/' + id, source: 'nitter' };
        }
      }
    } catch(e) { continue; }
  }
...
async function generateReply(tweetText, authorName, agnesKey) {
  if (!agnesKey) return 'Nice post @' + authorName + '!\uD0D3\uD8D2\uD7E4';
...
Balas tweet ini dengan 1-2 kalimat engaging:' }
  ...
  const data = await resp.json();
  return (data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content || 'Setuju!\uD0D3\uD8D2\uD7E4').trim().replace(/^"|"$/g, '').trim();
}
...
async function postReply(tweetId, text, h) {
  ...
  const resp = await fetch(url, {
    ...
    body: JSON.stringify({ text: text, reply: { in_reply_to_tweet_id: tweetId } }),
  });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.json({ status: 'replied', tweet: { id: tweet.id, text: tweet.text.slice(0,200), created_at: tweet.created_at }, reply: replyText, reply_id: posted.data && posted.data.id, url: tweet.url, source: tweet.source + ' + x-api-free' });
  } catch (err) {
