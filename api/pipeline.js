...
const {  AuthError, RateLimitError, RunLockError  } = require('./errors.js'');
...
const {  ThreadsClient  } = require('./threads-api.js'');
const {  OpenRouterClient  } = require('./openrouter.js'');
const {  buildMessages  } = require('./prompt.js'');
const {  findImage  } = require('./media.js'');
const {  pickRandom, dedupeBy, truncate, sanitizePost  } = require('./utils.js'');
...
const AI_FALLBACK_SEARCH_QUERIES = ['AI', 'ChatGPT', 'OpenAI'] as const;
...
function buildCrawlQueryPool(searchQueries: string[], shuffle = true): string[] {
...
async function collectSourcePosts(
  ...
  for (const query of crawlQueries) {
    if (allPosts.length >= minSourcePosts && successfulQueries.length >= minSourceQueries) break;
    ...
    if (uniqueAddedPosts.length > 0) successfulQueries.push(query);
...
function buildBalancedSourcePosts(
  queryResults: QueryCrawlResult[],
  ...
  while (balancedPosts.length < maxTotalPosts) {
    ...
    for (const queue of perQueryQueues) {
      ...
      pushedInRound = true;
      if (balancedPosts.length >= maxTotalPosts) break;
    }
