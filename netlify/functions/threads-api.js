...
const {  AuthError, RateLimitError, TransientError  } = require('./errors.js'');
const {  logger  } = require('./logger.js'');
const {  loadToken, saveToken, updateTokenUserId  } = require('./storage.js'');
...
class ThreadsClient {
  ...
  private async resolveThreadsUserId(): Promise<string> {
    ...
    if (this.resolvedThreadsUserId) return this.resolvedThreadsUserId;
  ...
  async exchangeCode(code: string): Promise<ShortLivedTokenResponse> {
    ...
    const result = await apiFetch<Partial<ShortLivedTokenResponse> & Pick<ShortLivedTokenResponse, 'access_token'>>(
      OAUTH_API_BASE + '/access_token',
      {
        method: 'POST',
  ...
  async getLongLivedToken(shortLivedToken: string, userId?: string): Promise<LongLivedTokenResponse> {
    const params = new URLSearchParams({
      grant_type: 'th_exchange_token',
      client_secret: this.config.threadsAppSecret,
      access_token: shortLivedToken,
    });
    const result = await apiFetch<LongLivedTokenResponse>(TOKEN_BASE + '/access_token?' + params.toString(), { method: 'GET' });
  ...
  async refreshToken(): Promise<void> {
    ...
    const result = await apiFetch<LongLivedTokenResponse>(TOKEN_BASE + '/refresh_access_token?' + params.toString(), { method: 'GET' });
    const expiresAt = new Date(Date.now() + result.expires_in * 1000);
  ...
  async getCurrentUserProfile(accessToken = this.getAccessToken()): Promise<ThreadsProfile> {
    ...
    return apiFetch<ThreadsProfile>(BASE_URL + '/me?' + params.toString(), {
  ...
  async keywordSearch(query: string, limit = 25): Promise<ThreadsPost[]> {
    ...
    const result = await this.withAuthRetry<SearchResult>(() =>
      apiFetch<SearchResult>(BASE_URL + '/keyword_search?' + params.toString(), {
  ...
  async createMediaContainer(text: string, imageUrl?: string): Promise<string> {
