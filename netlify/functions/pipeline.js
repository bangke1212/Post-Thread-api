...
export async function runPipeline(config, options = {}) {
  ...
  // Use pre-fetched link content (client-side already parsed)
  let linkContent = (config.linkContent || '').slice(0, 3000);
  if (linkContent) {
    logger.info('Using pre-fetched link content', { length: linkContent.length });
  }
